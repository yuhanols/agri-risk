...
\textbf{Market-level weather.} Weather variables are aggregated to the market level using the same shipment weights. As a result, market-level weather reflects conditions in the districts that are actively supplying the market in a given week, rather than a fixed geographic average. When production shifts seasonally from Salinas to Yuma, the corresponding market-level weather measures shift accordingly.
